...
int query_users(UiTestView *ui_test_view_data_buffer, PGconn *conn) {
    ...
    sql_query_absolute_path[0] = '\0';

    if (build_absolute_path(sql_query_absolute_path, "src/core/views/ui_test/users.sql") == -1) {
        free(sql_query_absolute_path);
        sql_query_absolute_path = NULL;
        return -1;
    }

    ssize_t query_file_size = calculate_file_size(sql_query_absolute_path);

    char *users_query;
    users_query = (char *)malloc(query_file_size * (sizeof *users_query) + 1);
    if (read_file(users_query, sql_query_absolute_path, query_file_size) == -1) {
        free(sql_query_absolute_path);
        sql_query_absolute_path = NULL;
        free(users_query);
        users_query = NULL;
...
int query_countries(UiTestView *ui_test_view_data_buffer, PGconn *conn) {
    ...
    if (build_absolute_path(sql_query_absolute_path, "src/core/views/ui_test/countries.sql") == -1) {
        free(sql_query_absolute_path);
        sql_query_absolute_path = NULL;
        return -1;
    }
    ...
    if (read_file(countries_query, sql_query_absolute_path, query_file_size) == -1) {
        free(sql_query_absolute_path);
        sql_query_absolute_path = NULL;
