...
int home_get(int client_socket, char *request) {
    ...
    /*
    if (te_multiple_substring_swap("for0", (size_t)num_columns, users_list,
    &response, (size_t)num_rows) == -1) { free_users_list(num_rows,
    num_columns, users_list); free(response); response = NULL; return -1;
    }
    */
