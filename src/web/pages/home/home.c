...
int web_page_home_get(int client_socket, char *request) {
    ...
    HomeView home_view_data;
    if (core_view_home(&home_view_data) == -1) {
        return -1;
    }
    ...
    for (i = 0; i < users_table_column_names_d1; i++) {
        for (j = 0; j < users_table_column_names_d2; j++) {
            users_table_column_names[i][j] = home_view_data.users_data.columns[i];
        }
    }
    ...
    if (te_multiple_substring_swap("{{ for->users_rows }}", "{{ end for->users_rows }}", 0, NULL, &response, home_view_data.users_data.rows) == -1) {
        free(response);
        response = NULL;
        return -1;
    }

    for (i = 0; i < home_view_data.users_data.rows; ++i) {
        char ***user;
        int user_d1 = 4;
        int user_d2 = 1;
        ...
        size_t id_length = strlen(home_view_data.users_data.users[i].id);
        user[0][0] = (char *)malloc(id_length * sizeof(char) + 1);
        strncpy(user[0][0], home_view_data.users_data.users[i].id, id_length);
        user[0][0][id_length] = '\0';

        size_t full_name_length = strlen(home_view_data.users_data.users[i].full_name);
        user[1][0] = (char *)malloc(full_name_length * sizeof(char) + 1);
        strncpy(user[1][0], home_view_data.users_data.users[i].full_name, full_name_length);
        user[1][0][full_name_length] = '\0';

        size_t email_length = strlen(home_view_data.users_data.users[i].email);
        user[2][0] = (char *)malloc(email_length * sizeof(char) + 1);
        strncpy(user[2][0], home_view_data.users_data.users[i].email, email_length);
        user[2][0][email_length] = '\0';

        size_t country_length = strlen(home_view_data.users_data.users[i].country);
        user[3][0] = (char *)malloc(country_length * sizeof(char) + 1);
        strncpy(user[3][0], home_view_data.users_data.users[i].country, country_length);
    ...
    if (te_multiple_substring_swap("{{ for->countries_rows }}", "{{ end for->countries_rows }}", 0, NULL, &response, home_view_data.countries_data.rows) == -1) {
        free(response);
        response = NULL;
        return -1;
    }

    for (i = 0; i < home_view_data.countries_data.rows; ++i) {
        char ***country;
        int country_d1 = 1;
        int country_d2 = 1;
        ...
        size_t country_name_length = strlen(home_view_data.countries_data.countries[i].country_name);
        country[0][0] = (char *)malloc(country_name_length * sizeof(char) + 1);
        strncpy(country[0][0], home_view_data.countries_data.countries[i].country_name, country_name_length);
    ...
    core_utils_view_home_free(&home_view_data);
